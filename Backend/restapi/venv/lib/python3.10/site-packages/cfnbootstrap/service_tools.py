...
class ServiceTool(object):
    ...
    def _detect_required_restart(self, serviceProperties, changes):
        ...
        if "packages" in serviceProperties and "packages" in changes:
            for manager, pkg_list in changes["packages"].items():
                if manager in serviceProperties["packages"]:
                    if frozenset(
                        serviceProperties["packages"][manager]
                    ) & frozenset(pkg_list):
                        return True
...
class WindowsServiceTool(ServiceTool):
    ...
    def apply(self, action, changes = collections.defaultdict(list)):
        ...
        try:
            for service, serviceProperties in action.items():
                handle = win32service.OpenService(manager, service, win32service.SERVICE_ALL_ACCESS)
                log.info("Enabled: %s, EnsureRunning: %s", serviceProperties["enabled"], serviceProperties["ensureRunning"])

                try:
                    if "enabled" in serviceProperties:
                        log.info("Setting service to AUTO_START or SERVICE_DEMAND_START")
                    ...
                    if self._detect_required_restart(serviceProperties, changes):
                        ...
                        log.debug("Service %s status: %s", service, status)
    ...
    def _set_service_startup_type(self, service, start_type):
        log.debug("Changing service startup type to %s. See https://learn.microsoft.com/en-us/windows/win32/api/winsvc/nf-winsvc-changeserviceconfiga for details", start_type)
        retValue = win32service.ChangeServiceConfig(service,
                                         ...
                                         None)
        if retValue == 0:
            errorValue = win32api.GetLastError()
            log.debug("Failed to change startup type. Return value: %s, error value from GetLastError: %s. See https://tiny.amazon.com/5h2c664u/learmicrenuswindwin3apiwins for more info.", retValue, errorValue)
