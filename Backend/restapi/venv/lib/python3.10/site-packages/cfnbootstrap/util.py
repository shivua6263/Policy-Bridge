...
class EtagCheckedResponse(object):
    ...
    def __init__(self, response):
        ...
        # the md5() constructor is faster but only supports usedforsecurity in Python 3.9, so try new() on failure
        ...
        if self._etag:
            try:
                ...
                log.warning(
                    'Failed to retrieve md5 hash object from hashlib. Commonly this means the algorithm provider, likely OpenSSL\'s libcrypto library, disabled md5 when configured to be FIPS compliant. Skipping md5 checksum verification of S3 objects.', exc_info=True)
...
def _get_metadata_response(path, token, _timeout=REQUEST_TIMEOUT):
    if not token:
