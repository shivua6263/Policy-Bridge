...
S3_PATTERN = r'https://s3(-fips)?([\.-][a-z0-9-]+)\.amazonaws\.com(.*)$'
...
def get_endpoint_for_url(service, unparsed_url):
    ...
    if service == "AmazonS3":
        ...
        if m:
            return Endpoint.from_region(S3_FORMAT, m.group(3), m.group(2) is None)
        else:
            m = re.match(S3_PATTERN, unparsed_url)
            if m:
                return Endpoint.from_region(S3_FORMAT, m.group(2), m.group(1) is None)
...
class Endpoint(object):
    ...
    @classmethod
    def from_region(cls, endpoint_format, region, is_regular_processing=True):
        if region[:1] not in ['.', '-']:
            region = '.' + region
        s3_subdomain = region if is_regular_processing else '-fips' + region
        endpoint_tld = '.cn' if region.startswith('cn') else ''
        return cls(region[1:], endpoint_format % (s3_subdomain, endpoint_tld))
    ...
    def matches_url(self, unparsed_url):
        return (
            urlparse(unparsed_url)
            .netloc.lower()
            .endswith(self.hostname)
        )

    def get_subdomain_prefix(self, unparsed_url):
        netloc = urlparse(unparsed_url).netloc.lower()
