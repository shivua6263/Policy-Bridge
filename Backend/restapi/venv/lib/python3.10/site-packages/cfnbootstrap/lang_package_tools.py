...
class PythonTool(object):
    ...
    def apply(self, action, auth_config=None):
        ...
        for pkg in action:
            if not action[pkg] or isinstance(action[pkg], str):
        ...
        log.info("Attempting to install %s via pip", pkgs)

        result = ProcessHelper(['pip3', 'install'] + pkgs).call()

        if result.returncode:
            log.error("pip failed. Output: %s", result.stdout)
            raise ToolError("Could not successfully install python packages", result.returncode)
        else:
            log.info("pip installed %s", pkgs)
            log.debug("pip output: %s", result.stdout)
...
class GemTool(object):
    ...
    def _install_gem(self, pkg, ver=None):
        ...
        if self._gem_is_installed(pkg, ver):
            ...
            install_command = ['gem', 'install', '-b', '--no-document', pkg];
