...
class SourcesTool(object):
    ...
    _github_pattern = re.compile(
        r'^https?://github.com/.*?/(zipball|tarball)/.*$')
    ...
    def apply(self, action, auth_config):
        ...
        for (path, archive) in sorted(action.items(), key=lambda pair: pair[0]):
            ...
            if TarWrapper.is_compatible(archive_file):
                ...
                raise ToolError(
                    "Unsupported source file (not zip or tarball): %s" % archive)

            log.debug(
                "Checking to ensure that all archive members fall under path %s" % path)
    ...
    def _perform_github_magic(self, archive):
        ...
        try:
            ...
            if len(tempmembers) != 1:
                log.debug(
                    "GitHub magic is not possible; archive does not contain exactly one directory")
                return archive
            else:
        ...
        for member in archive.files():
            if os.path.isabs(member):
                prefix = os.path.commonprefix(
                    [os.path.normcase(os.path.normpath(member)), normalized_parent])
            else:
                prefix = os.path.commonprefix([os.path.normcase(os.path.normpath(
                    os.path.join(normalized_parent, member))), normalized_parent])
...
class TarWrapper(object):
    ...
    def is_compatible(cls, f):
        try:
            ...
            log.debug("Exception opening tar: %s", e)
            return False
