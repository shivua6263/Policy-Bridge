...
class YumTool(object):
    ...
    def apply(self, action, auth_config=None):
        ...
        if not action.keys():
            ...
            return pkgs_changed
        # Clean existing caches before making the yum cache.
        clean_result = LoggingProcessHelper(['yum', '-y', 'clean', 'metadata'], name=u'yum clean metadata').call()
        if clean_result.returncode:
            log.warn("Yum metadata clean failed. Will attempt to continue bootstapping anyway. Output: %s", clean_result.stdout)
