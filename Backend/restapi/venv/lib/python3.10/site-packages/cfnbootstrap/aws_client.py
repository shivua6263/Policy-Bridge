...
class CFNSigner(Signer):
    ...
    def sign(self, verb, base_url, params, creds, in_headers=None, timestamp=None):
        ...
        document = util.get_instance_identity_document().encode('utf-8')
        signature = util.get_instance_identity_signature()

        new_headers = dict({} if in_headers is None else in_headers)
        new_headers['Authorization'] = 'CFN_V1 %s:%s' % (
            base64.b64encode(document).decode('utf-8'), signature.replace('\n', ''))
...
class V4Signer(Signer):
    ...
    def sign(self, verb, base_url, params, creds, in_headers=None, timestamp=None):
        ...
        scope = timestamp_short + '/' + self._region + \
            '/' + self._service + '/' + self._terminator
        ...
        new_headers['Host'] = urlsplit(base_url).netloc
        if verb == 'POST':
            new_headers['Content-type'] = 'application/x-www-form-urlencoded'

        canonical_request = verb + '\n'
        canonical_request += self._canonicalize_uri(base_url) + '\n'
        canonical_request += (self._canonicalize_query(params)
                              if verb == 'GET' else '') + '\n'

        (canonical_headers, signed_headers) = self._canonicalize_headers(new_headers)
        canonical_request += canonical_headers + '\n' + signed_headers + '\n'
        canonical_request += hashlib.sha256((Client.construct_query(params) if verb == 'POST' else '').encode('utf-8'))\
            .hexdigest()
        ...
        derived_key = hmac.new(("AWS4" + creds.secret_key).encode('utf-8'),
                               timestamp_short.encode('utf-8'), hashlib.sha256).digest()
        derived_key = hmac.new(derived_key, self._region.encode(
            'utf-8'), hashlib.sha256).digest()
        derived_key = hmac.new(derived_key, self._service.encode(
            'utf-8'), hashlib.sha256).digest()
        ...
        new_headers['Authorization'] = 'AWS4-HMAC-SHA256 Credential=%s,SignedHeaders=%s, Signature=%s' % (
    ...
    def _canonicalize_query(self, params):
        ...
        encoded_pairs = ((quote(entry[0], '~'), quote(
            entry[1], '~') if len(entry) > 1 else '') for entry in params.items())
...
class AwsQueryError(util.RemoteError):
    ...
    def __init__(self, status_code, error_code, error_type, msg):
        ...
        super(AwsQueryError, self).__init__(status_code,
                                            "%s: %s" % (error_code, msg), retry_mode)
...
class Client(object):
    ...
    def construct_query(sign_data):
        ret_str = ""
        for k, vs in sorted(sign_data.items(), key=operator.itemgetter(0)):
            if isinstance(vs, list):
                for v in sorted(vs):
                    ret_str += '&'.join(quote(k, safe='~') +
                                        '=' + quote(v, safe='~'))
            else:
                if ret_str:
                    ret_str += '&'
                ret_str += quote(k, safe='~') + \
                    '=' + quote(vs, safe='~')
    ...
    def _get_xml_extractor(xmlns):
        def _extract_xml_message(resp):
            try:
                eDoc = ElementTree.ElementTree(
                    file=StringIO(resp.content)
                )
                code = eDoc.findtext("{%s}Error/{%s}Code" % (xmlns, xmlns))
                error_type = eDoc.findtext(
                    "{%s}Error/{%s}Type" % (xmlns, xmlns)
                )
                message = eDoc.findtext(
                    "{%s}Error/{%s}Message" % (xmlns, xmlns)
                )
    ...
    def _call(self, params, endpoint=None, request_credentials=None, verb='GET', timeout=None):
        ...
        req = self._signer.sign(verb, base, params, creds, {
                                "Accept": accept_type})
    ...
    def _construct_user_agent(self):
        ...
        try:
            ...
            log.debug(
                "Constructing user agent failed with error message: " + str(e))
