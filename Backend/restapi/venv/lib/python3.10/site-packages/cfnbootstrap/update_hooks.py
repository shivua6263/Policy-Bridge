...
import tempfile
import time
from cfnbootstrap.util import Credentials
import json
...
def parse_config(config_path):
    ...
    if not os.path.isfile(main_conf_path):
        raise ValueError(
            "Could not find main configuration at %s" % main_conf_path)
    ...
    if main_config.has_option('main', 'role'):
        credentials = util.RoleBasedCredentials(
            main_config.get('main', 'role'))
    elif main_config.has_option('main', 'credential-file'):
        try:
            credentials = util.extract_credentials(
                main_config.get('main', 'credential-file'))
        except IOError as e:
            raise ValueError(
                "Could not retrieve credentials from file:\n\t%s" % e.strerror)
    else:
        ...
        for hook_file in os.listdir(additional_hooks_path):
            if os.path.isfile(os.path.join(additional_hooks_path, hook_file)):
                additional_files.append(os.path.join(
                    additional_hooks_path, hook_file))

    hooks_config = ConfigParser.ConfigParser()
    files_read = hooks_config.read(
        [os.path.join(config_path, 'hooks.conf')] + additional_files)

    if not files_read:
        raise ValueError("No hook configurations found at %s or %s." % (
            os.path.join(config_path, 'hooks.conf'), additional_hooks_path))
    ...
    for section in hooks_config.sections():
        ...
        triggers = [s.strip()
                    for s in hooks_config.get(section, 'triggers').split(',')]
        ...
        if hook.is_cmd_hook():
            if hooks_config.has_option(section, 'singleton'):
                hook.singleton = util.interpret_boolean(
                    hooks_config.get(section, 'singleton'))
            if hooks_config.has_option(section, 'send_result'):
                hook.send_result = util.interpret_boolean(
                    hooks_config.get(section, 'send_result'))
...
class AutoRefreshingCredentialsProvider(object):
    ...
    def refresh(self):
        with self._refresh_lock:
            ...
            try:
                self._creds = self._cfn_client.get_listener_credentials(
                    self._stack_name, self._listener_id)
                self.listener_expired = False
            except IOError as e:
                if hasattr(e, 'error_code') and 'ListenerExpired' == e.error_code:
                    ...
                    self.listener_expired = False
                    log.exception(
                        "IOError caught while refreshing credentials")
            except Exception:
                self.listener_expired = False
            ...
            expiration = calendar.timegm(
                self._creds.expiration.utctimetuple()) if self._creds else now
            ...
            log.info(
                "Scheduling next credential refresh in %s seconds", next_refresh)
...
class CmdProcessor(object):
    ...
    def register(self):
        try:
            self.queue_url = self.cfn_client.register_listener(
                self.stack_name, self.listener_id).queue_url
        except AwsQueryError as e:
            if e.error_code == 'ValidationError':
                raise FatalUpdateError(
                    'Terminal failure registering listener: %s' % str(e))
            raise
    ...
    def _create_storage_dir(self):
        if os.name == 'nt':
            self.storage_dir = os.path.expandvars(
                r'${SystemDrive}\cfn\cfn-hup\data')
        else:
            self.storage_dir = '/var/lib/cfn-hup/data'
        if not os.path.isdir(self.storage_dir):
            log.debug("Creating %s", self.storage_dir)
            try:
                os.makedirs(self.storage_dir, 0o700)
            except OSError:
                log.warn(
                    "Could not create %s; using temporary directory", self.storage_dir)
                self.storage_dir = tempfile.mkdtemp()

    def _command_completed(self, msg):
        now = datetime.datetime.utcnow()
        cmd_time = now.replace(hour=0, minute=0, second=0,
        ...
        with self._commands_run_lock:
            ...
            try:
                keys_to_delete = []
                for key in self._commands_run["by_day"].keys():
    ...
    def _delete_message(self, receipt_handle):
        try:
            self.sqs_client.delete_message(
                self.queue_url, receipt_handle, request_credentials=self._creds_provider.credentials)
        except Exception:
            log.exception(
                "Could not delete message for handle %s", receipt_handle)
    ...
    def process(self):
        if self.queue_url is None:
            raise FatalUpdateError(
                "Cannot process command hooks before registering")
                    ...
                    if self._get_id(cmd_msg) not in self._currently_running and not self._already_run(cmd_msg):
                        ...
                        if self.multi_threaded:
                            thread = threading.Thread(
                                target=self._process_msg, args=(cmd_msg, msg.receipt_handle))
                            thread.daemon = True
    ...
    def _process_msg(self, cmd_msg, receipt_handle):
        ...
        try:
            ...
            if expiration < datetime.datetime.utcnow():
                ...
                log.info("Received command %s (invocation id: %s)",
                         cmd_msg['CommandName'], cmd_msg['InvocationId'])
    ...
    def _run_hook(self, hook, cmd_msg):
        if hook.singleton:
            ...
            if leader == self.listener_id:
                ...
                log.info(
                    "This listener is not the leader; %s is the leader.", leader)
                return True
        ...
        result = ProcessHelper(
            action, stderr=subprocess.PIPE, env=action_env).call()
        ...
        if result.returncode:
            ...
            result_msg['Message'] = result_stderr if len(
                result_stderr) <= 1024 else result_stderr[0:500] + '...' + result_stderr[-500:]
        else:
            result_stdout = result.stdout.rstrip()
            if len(result_stdout) > 1024:
                log.error(
                    "stdout for %s was greater than 1024 in length, which is not allowed", hook.name)
                result_msg['Status'] = 'FAILURE'
                result_msg['Message'] = 'Result data was longer than 1024 bytes. Started with: ' + \
                    result_stdout[0:100]
            else:
        ...
        try:
            self.sqs_client.send_message(result_queue, json.dumps(
                result_msg), request_credentials=self._creds_provider.credentials)
        except Exception:
    ...
    def _hooks_by_path(self, hooks):
        ...
        for hook in hooks:
            if hook.path in ret_hooks:
                raise FatalUpdateError(
                    "Multiple hooks for the same command (%s)" % hook.path)
...
class HookProcessor(object):
    ...
    def __init__(self, hooks, stack_name, client):
        ...
        if not os.path.isdir(self.dir):
            ...
            try:
                ...
                log.warn(
                    "Could not create %s; using temporary directory", self.dir)
    ...
    def process(self):
        ...
        for hook in self.hooks:
            try:
                ...
                log.exception(
                    "Exception caught while running hook %s", hook.name)
    ...
    def _process_hook(self, hook, metadata):
        ...
        if 'post.add' in hook.triggers and not old_data and new_data:
            log.info(
                "Previous state not found; action for %s will be run", hook.name)
            ...
            log.info(
                "Data has changed from previous state; action for %s will be run", hook.name)
        else:
        ...
        if result.returncode:
            log.warn("Action for %s exited with %s; will retry on next iteration",
                     hook.name, result.returncode)
        else:
            metadata[hook.name + '|' +
                     hook.path] = self._jsonConverter.serialize(new_data)
    ...
    def _retrieve_path_data(self, path):
        ...
        if len(parts) < 2:
            raise FatalUpdateError(
                "Unsupported path: paths must be in the form Resources.<LogicalResourceId>(.Metadata|PhysicalResourceId)(.<optional Metadata subkey>). Input: %s" % path)

        if parts[0].lower() != 'resources':
            raise FatalUpdateError(
                'Unsupported path: only changes to Resources are supported (path: %s)' % path)

        if len(parts) == 2:
            resourcePart = None
        elif parts[2].lower() not in ['metadata', 'physicalresourceid']:
            raise FatalUpdateError(
                "Unsupported path: only Metadata or PhysicalResourceId can be specified after LogicalResourceId (path: %s)" % path)
        else:
        ...
        if logical_id not in self._resource_cache:
            self._resource_cache[logical_id] = self.client.describe_stack_resource(
                logical_id, self.stack_name)

        resource = self._resource_cache[logical_id]
        status = resource.resourceStatus

        if status and status.endswith('_IN_PROGRESS'):
            log.debug("Skipping resource %s in %s as it is in status %s",
                      logical_id, self.stack_name, status)
            raise InFlightStatusError('%s in %s is in status %s' % (
                logical_id, self.stack_name, status))

        if resourcePart == 'metadata':
            if not resource.metadata:
                log.warn("No metadata for %s in %s",
                         logical_id, self.stack_name)
                return None
