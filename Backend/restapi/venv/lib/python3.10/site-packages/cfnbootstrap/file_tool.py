...
class FileTool(object):
    ...
    def is_abs_compat(cls, path):
        ...
        #  keep base logic for backward compatibility
        if os.path.isabs(path):
            return True
            

        if sys.platform.startswith("win"):
            # Allow historical Unix-style absolute paths that begin with '/'
            # (Python <=3.11 treated these as absolute on Windows)
            return ntpath.isabs(path) or path.startswith(("/"))
    ...
    def apply(self, action, auth_config):
        ...
        for (filename, attribs) in sorted(action.items(), key=lambda pair: pair[0]):
            if not self.is_abs_compat(filename):
                raise ToolError(
