...
class CommandTool(object):
    ...
    def apply(self, action):
        ...
        for name in sorted(action.keys()):
            ...
            if "defaults" in attributes:
                ...
                defaultsResult = ProcessHelper(
                    attributes['defaults'], stderr=subprocess.PIPE).call()
                log.debug(u"Defaults script for %s output: %s",
                          name, defaultsResult.stdout)
                if defaultsResult.returncode:
                    log.error(u"Defaults script failed for %s: %s",
                              name, defaultsResult.stderr)
                    raise ToolError(
                        u"Defaults script for command %s failed" % name)
            ...
            if not "command" in attributes:
                ...
                raise ToolError(
                    u"%s does not specify the 'command' attribute, which is required" % name)

            cwd = os.path.expanduser(
                attributes["cwd"]) if "cwd" in attributes else None
            env = attributes.get("env", None)

            if "test" in attributes:
                log.debug(u"Running test for command %s", name)
                test = attributes["test"]
                testResult = LoggingProcessHelper(
                    test, name=u'Test for Command %s' % name, env=env, cwd=cwd).call()
                log.debug(u"Test command output: %s", testResult.stdout)
                if testResult.returncode:
                    log.info(u"Test failed with code %s",
            ...
            if "runas" in attributes:
                if os.name == 'nt':
                    raise ToolError(
                        u'Command %s specified "runas", which is not supported on Windows' % name)

                if isinstance(cmd_to_run, str):
                    cmd_to_run = u'su %s -c %s' % (
                        attributes['runas'], cmd_to_run)
                else:
                    cmd_to_run = ['su', attributes['runas'], '-c'] + cmd_to_run

            commandResult = LoggingProcessHelper(
                cmd_to_run, name=u'Command %s' % name, env=env, cwd=cwd).call()

            if commandResult.returncode:
                log.error(u"Command %s (%s) failed",
                          name, attributes["command"])
                log.debug(u"Command %s output: %s", name, commandResult.stdout)
    ...
    def get_wait(cls, cmd_options):
        wait = cmd_options.get('waitAfterCompletion',
                               60 if os.name == 'nt' else 0)
        if isinstance(wait, str) and 'forever' == wait.lower():
            return -1
        return int(wait)
