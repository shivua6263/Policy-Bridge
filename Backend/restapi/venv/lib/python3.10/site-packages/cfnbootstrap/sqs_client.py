...
class SQSClient(aws_client.Client):
    ...
    def __init__(self, credentials, url=None, region='us-east-1', proxyinfo=None):
        ...
        super(SQSClient, self).__init__(credentials, False, endpoint, signer=signer,
                                        xmlns='http://queue.amazonaws.com/doc/%s/' % SQSClient._apiVersion, proxyinfo=proxyinfo)
    ...
    def _fix_endpoint(self, unparsed_url):
        ...
        m = re.match(endpoint_tool.SQS_PATTERN, unparsed_url) or re.match(
            endpoint_tool.SQS_LEGACY_PATTERN, unparsed_url)
        if m:
    ...
    def regionForEndpoint(cls, parsed_url):
        ...
        m = re.match(endpoint_tool.SQS_PATTERN, parsed_url) or re.match(
            endpoint_tool.SQS_LEGACY_PATTERN, parsed_url)
        if m:
            return m.group(1)
        return None
    ...
    def receive_message(self, queue_url, attributes=None, max_messages=1, visibility_timeout=None,
        ...
        params = {"Action": "ReceiveMessage", "Version": SQSClient._apiVersion,
        ...
        response_content = self._call(
            params,
            queue_url,
            request_credentials,
            timeout=wait_time + 3 if wait_time else None,
        ).content.decode("utf-8")
        return Message._parse_list(
            StringIO(response_content), self._xmlns
        )
    ...
    def send_message(self, queue_url, message_body, delay_seconds=None, request_credentials=None):
        ...
        params = {"Action": "SendMessage",
                  "Version": SQSClient._apiVersion, "MessageBody": message_body}

        if delay_seconds:
            params["DelaySeconds"] = delay_seconds

        root = ElementTree.ElementTree(file=StringIO(self._call(
            params, queue_url, request_credentials, verb='POST').content.decode("utf-8")))
        message_id = root.findtext(
            '{%s}SendMessageResult/{%s}MessageId' % (self._xmlns, self._xmlns))
        md5_of_body = root.findtext(
            '{%s}SendMessageResult/{%s}MD5OfMessageBody' % (self._xmlns, self._xmlns))
    ...
    def delete_message(self, queue_url, receipt_handle, request_credentials=None):
        ...
        params = {"Action": "DeleteMessage",
                  "Version": SQSClient._apiVersion, "ReceiptHandle": receipt_handle}
...
class Message(object):
    ...
    def _parse_list(cls, data, xmlns):
        ...
        msgs = root.findall(
            '{%s}ReceiveMessageResult/{%s}Message' % (xmlns, xmlns))
    ...
    def _from_elem(cls, elem, xmlns):
        ...
        for attrib in elem.findall('{%s}Attribute' % xmlns):
            attribs[attrib.findtext('{%s}Name' % xmlns)] = attrib.findtext(
                '{%s}Value' % xmlns)
