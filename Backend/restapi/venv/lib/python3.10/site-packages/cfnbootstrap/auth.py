...
# ==============================================================================
from cfnbootstrap import endpoint_tool, util
import operator
...
import logging
import re
from urllib.parse import urlparse, urlsplit, urljoin, parse_qs, quote, unquote
...
def _extract_bucket_from_url(unparsed_url):
    ...
    if not bucket:
        ...
        return urlparse(unparsed_url).path[1:].partition("/")[0]
    else:
...
class S3Signer(object):
    ...
    def handle_redirect(self, r, **kwargs):
        if r.is_redirect and 'Location' in r.headers:
            if _extract_bucket_from_url(r.headers['Location']) is not None:
                log.warn("Handling redirect to S3 location: %s",
                         r.headers['Location'])
                ...
                log.error('S3 redirected to non-S3 url: %s',
                          r.headers['Location'])
    ...
    def sign(self, req):
        ...
        full_url = unquote(
            req.full_url if hasattr(req, "full_url") else req.url
        )
        ...
        if not region:
            ...
            if endpoint.is_default:
                log.warn(
                    'Falling back to Signature Version 2 as no region was specified in S3 URL')
        ...
        hashed_payload = hashlib.sha256(
            (req.body if req.body is not None else '').encode('utf-8')).hexdigest()
        ...
        canonical_request += (
            self._canonicalize_query(parse_qs(parsed.query, True))
            + "\n"
        )
        ...
        derived_key = hmac.new(("AWS4" + self._creds.secret_key).encode(
            'utf-8'), timestamp_short.encode('utf-8'), hashlib.sha256).digest()
        derived_key = hmac.new(derived_key, region.encode(
            'utf-8'), hashlib.sha256).digest()
        derived_key = hmac.new(derived_key, 's3'.encode(
            'utf-8'), hashlib.sha256).digest()
        derived_key = hmac.new(derived_key, "aws4_request".encode(
            'utf-8'), hashlib.sha256).digest()
        ...
        req.headers[_AUTHORIZATION_HEADER] = 'AWS4-HMAC-SHA256 Credential=%s,SignedHeaders=%s,Signature=%s' % (
    ...
    def _canonicalize_uri(self, uri):
        ...
        if not split.path:
            return "/"
        path = urlsplit(
            urljoin("http://foo.com", split.path.lstrip("/"))
        ).path
    ...
    def _canonicalize_headers(self, headers):
        canon_headers = {}
        for key, value in ((key.lower(), re.sub(r'(?su)[\s]+', ' ', value).strip()) for key, value in headers.items()):
            if key in canon_headers:
                canon_headers[key] = canon_headers[key] + ',' + value
            else:
                canon_headers[key] = value

        sorted_entries = sorted(canon_headers.items(),
...
class S3V2Signer(object):
    ...
    def sign(self, req):
        if 'Date' not in req.headers:
            req.headers[_AMZ_DATE_HEADER] = datetime.datetime.utcnow().replace(
                microsecond=0).strftime("%a, %d %b %Y %H:%M:%S GMT")
        ...
        signed = base64.encodebytes(
            hmac.new(self._creds.secret_key.encode('utf-8'), stringToSign.encode('utf-8'), hashlib.sha1).digest())\
            .strip()\
            .decode('utf-8')
    ...
    def _canonicalize_headers(self, req):
        headers = [(hdr.lower(), val) for hdr, val in req.headers.items()
                   if hdr.lower().startswith('x-amz')]
        ...
        for key, config in model.items():
            ...
            if 's3' == configType.lower():
                ...
                if 'accessKeyId' in config and 'secretKey' in config:
                    ...
                    log.warn(
                        'S3 auth requires either "accessKeyId" and "secretKey" or "roleName"')
                    continue

                self._auths[key] = auth_impl

                if 'buckets' in config:
                    buckets = [config['buckets']] if isinstance(
                        config['buckets'], str) else config['buckets']
                    for bucket in buckets:
                        s3Auth.add_auth_for_bucket(bucket, auth_impl)

            elif 'basic' == configType.lower():
                self._auths[key] = HTTPBasicAuth(
                    config.get('username'), config.get('password'))
                if 'uris' in config:
                    if isinstance(config['uris'], str):
                        basicAuth.add_password(config['uris'], config.get(
                            'username'), config.get('password'))
                    else:
                        for u in config['uris']:
                            basicAuth.add_password(u, config.get(
                                'username'), config.get('password'))
            else:
